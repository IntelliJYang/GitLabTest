...
class TestItem(object):
    ...
    def __init__(self, row):
        ...
        self.returned_val = None  # variable name of the return value
        ...
        if len(s_timeout) == 0:
            if self.function in AUTO_TIMEOUT_FUNCS:
                if 'PARAM1' in self.row and self.row['PARAM1'].isdigit():
                    self.timeout = max(2 * int(self.row['PARAM1']), 500)
                else:
        ...
        if self.low_limit:  # we actually have limits
            if self.low_limit == self.high_limit and not ispdata(self.low_limit):# there is one case, lower limit is equal to high limt, but value type is digital, [0.0005,0.0005], test data is 0.0005000 Aaron Tong 0510/2019
    ...
    def between(self, val):
        ...
        try:
            ...
            #                                 + ': ' + e.message)
            return False
        return self.low_limit <= val <= self.high_limit

    def __eq__(self, _tid, **kwargs):
        if 'group' in kwargs:
            return self.tid == _tid and self.group == kwargs['group']
        else:
    ...
    def should_run_by_filter(self, variables):
        if self.filters:
            for filter_name in self.filters.keys():
                if filter_name not in variables or variables[filter_name] != self.filters.get(filter_name):
                    return False
        return True
...
class TestSequence(object):
    ...
    def __init__(self, sequence_db=None):
        ...
        if '.csv' == os.path.splitext(db_name)[1]:
    ...
    @staticmethod
    def is_group_run_function(function_name):
                ...
                if len(test.kwargs) > 0:
                    for variable_name in test.kwargs.keys():
                        if variable_name not in self.variables:
                            raise KeyError('in test ' + test.tid + ' variable ' + variable_name +
