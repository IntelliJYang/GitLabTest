...
class LoggerServer(Thread):
    ...
    def __init__(self, publisher, site_count=6, is_auditmode=False,url='tcp://localhost:'):
        ...
        if root:
            ...
            for logger in loggers:
                ...
                if f_log.alias == 'Pudding':
                    f_log.server = self
                    if self.auditmode:
                        f_log.priority = pypudding.IP_PRIORITY_STATION_CALIBRATION_AUDIT
                ...
                for sub in subs:
                    ...
                    if len(channel) > 0:
                        ...
                        if not channel:
                            print str(channel) + ' not defined in zmqports.py'
                            return False
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--audit_mode', help='audit and grr mode', action='store_true',default=False)  # specify audit and GRR mode
    args = parser.parse_args()

    ctx = zmq.Context()
    publisher = ZmqPublisher(ctx, "tcp://*:" + str(zmqports.LOGGER_PUB), 'Logger')
    logger_server = LoggerServer(publisher,is_auditmode=args.audit_mode)
