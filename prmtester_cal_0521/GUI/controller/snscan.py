...
class SnScanController(object):
    ...
    def scan_confirm(self):
        ...
        # if self.fixture.fixture_status():
        #     if not self.check_if_repeat():
        #         if self.check_if_finish():
        #             self.collect_e_travelers()
        # else:
        #     QMessageBox.warning(QMessageBox(), "WARINGING", "Please push Fixture IN First\n请先把治具推进去")
            ...
            for j in range(constant.SLOTS):
                with open("/tmp/HeartBeat_Slot{}.txt".format(j + 1), "r") as f:
                    ...
                    if result == "1":
                        ...
                        self.view.sn_widgets_list[i][j].select(False)
                        QMessageBox.warning(QMessageBox(), "WARINGING",
                                            "Please check slot{} test engine status\n请检查通道{}测试状态".format(j + 1, j + 1))
        self.view.set_first_focus()
...
class SnScanView(QFrame):
    ...
    def set_first_focus(self):
        self.sn_widgets_list[0][0].mlb.setFocus()
