...
class FileLogger(ReportLogger):
    ...
    def __init__(self, log_folder='/tmp', site_count=6, sub_per_site=1, publisher=NoOpPublisher(), suffix='', level=levels.DEBUG):
        ...
        self.context_buffer = [['' for sub in range(sub_per_site)] for site in range(site_count)]
    ...
    def on_sequence_start(self, site, data):
        ...
        for sub in range(self.sub_per_site):
            ...
            # else:
            #     Site_ID = 'RIGHT'
            # filename = '_'.join([self.product, self.station_type, '{}-DUT'.format(Site_ID), mid_name, ts, self.suffix])
